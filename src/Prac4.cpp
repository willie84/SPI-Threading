...
long previousTime=0; //used to refere the previous time that was read.
...
void play_pause_isr(void){
   ...
   if(timepassed-previousTime > 100){
    playing=!playing;
 }
...
int setup_gpio(void){
    ...
    pinMode(PLAY_BUTTON, INPUT);  //setting play button as input

    pullUpDnControl(PLAY_BUTTON, PUD_UP);//putting pull up resustors

    pullUpDnControl(STOP_BUTTON, PUD_UP);//putting pull dow for the stop button

    wiringPiISR(STOP_BUTTON,INT_EDGE_FALLING,stop_isr);//adding ISR for stop button
    wiringPiISR(PLAY_BUTTON,INT_EDGE_FALLING,play_pause_isr);//adding ISR for play button
    //setting up the SPI interface
    //TODO
   wiringPiSPISetup(SPI_CHAN,SPI_SPEED);//setting SPI INterface
    return 0;
}
...
void *playThread(void *threadargs){
	...
	continue;
        ...
        //TODO
	wiringPiSPIDataRW(SPI_CHAN,buffer[bufferReading][buffer_location], 10);//length is 10 bits the bit widths
...
int main(){
    ...
    // Have a loop to read from the file
	 while((ch = fgetc(filePointer)) != EOF){
       ...
       char numeral=fgetc(filePointer);
       char numeralA= ((numeral >> 6) & 0xff) | 0b0 << 7 | 0b0 << 6 | 0b1 << 5 | 0b1 << 4;       //Set config bits for first 8 bit packet and OR with upper bits
       char numeralB = numeral<< 2 & 0b11111100;//using the and operand
	 buffer[bufferWriting][counter][0] = numeralA; //TODO
        //Set next 8 bit packet
        buffer[bufferWriting][counter][1] = numeralB; //TODO
